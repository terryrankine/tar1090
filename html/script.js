...
(function (global, jQuery, TAR) {
    ...
    // LineString with 'points' segments from 'start' ([lon, lat]) along 'bearing' (degrees) for 'distance' meters
    utils.make_geodesic_line = function (start, bearing, distance, points) {
        const rad = bearing * Math.PI / 180.0;
        let geom = new ol.geom.LineString([start]);
        for (let i = 1; i <= points; ++i) {
            geom.appendCoordinate(utils.geodesic_destination(start, rad, distance * i / points));
        }
        return geom;
    }
